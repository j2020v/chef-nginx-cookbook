...
describe 'node_2::default' do
  context 'When all attributes are default, on Ubuntu 18.04' do
    ...
    it 'should create a symlink from sites-available to sites-enabled' do
      expect(chef_run).to create_link("/etc/nginx/sites-enabled/proxy.conf").with_link_type(:symbolic)
    end

    it 'should delete that symlink from the default config' do
    ...
    it 'should create proxy.conf template in /etc/nginx/sites-available with port 3000 and proxy_port 3030' do
